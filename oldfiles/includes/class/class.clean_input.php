...
define('TYPE_NAME',					1902);
define('TYPE_EMAIL',				1903);
define('TYPE_CAPTCHA',				1904);
define('TYPE_PASSWORD',				1905);
define('TYPE_TITLE',				1907);
define('TYPE_BODY',					1908);
define('TYPE_CHAR',					1909);
define('TYPE_PERCENT',				1910);
define('TYPE_DECIMAL',				1911);
define('TYPE_NOT_PARENT',			1912);
define('TYPE_META_KEYWORDS',		1912);
define('TYPE_META_DESCRIPTIONS',	1912);
define('TYPE_LINK',					1912);
define('TYPE_PAGE_TITLE',			1912);
define('TYPE_UPLOAD_CONTENT',		1913);
define('TYPE_LICENSE',				1914);
define('TYPE_COUNTRY',				1915);
define('TYPE_PROVINCE',				1916);
...
class Input_Cleaner
{
	...
	*/
	function Input_Cleaner(&$registry)
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')
		{
		...
		define('REQ_PROTOCOL', (!empty($_SERVER['https']) && (($_SERVER['HTTPS'] == 'on' || $_SERVER['HTTPS'] == '1')) ? 'https' : 'http'));
	...
	function required($data, $input = array(), $exceptions = array())
	{
	   $input = empty($input) ? $this->registry->GPC : $input;
	   $error_msg = array();
	   ...
	   foreach ($data as $key => $value)
	   {
	      ...
	      switch ($value)
		  {
			 case TYPE_COUNTRY:
			      if (strlen($result) <= 0)
				  	  $error_msg[$key] = 'Select Country';
			 	break;
				
			 case TYPE_PROVINCE:
			      if (strlen($result) <= 0)
				  	  $error_msg[$key] = 'Select Province/State';
			 	break;
				
			 case TYPE_UPLOAD_CONTENT:  
			      if (!upload_success('local') && (empty($result) || strtolower($result == 'http://')))
						eval(fetch_error_msg('must_select_file_or_url', $key));
				  else if (upload_success('local') && strlen($result) > 7 && strpos($result, 'http://') !== false)
						eval(fetch_error_msg('cannot_select_file_and_url', $key));					  
				  else if (!empty($result) && strpos($result, 'http://') === false)
						eval(fetch_error_msg('url_must_contain_http', $key));				  
			 	break;
			
			 case TYPE_LICENSE:
			 	  if ($_POST[$key] != 'checked')
				  	   eval(fetch_error_msg('must_accept_license', $key));
			 	break;	
				
			 case TYPE_FILE:
			 	  if (!upload_success($key))
						eval(fetch_error_msg('must_select_file', $key));	
				break;
				
			 case TYPE_NOT_PARENT:
			        $value = intval($result);
					if ($value <= 0)		 
						eval(fetch_error_msg('cannot_be_parent', $key));	
			 	break;
				
			 case TYPE_DECIMAL:
					if ($result < 0 || !is_float($result) || $result > 1)		 
						eval(fetch_error_msg('decimal_required', $key));	
			 	break;
				
			 case TYPE_PERCENT:
			        $value = intval($result);
					if ($value <= 0 || $value > 100)		 
						eval(fetch_error_msg('percentage_required', $key));	
			 	break;
			
			 case TYPE_CHAR:
			 		if (strlen($result) > 1 || empty($result))	
						eval(fetch_error_msg('not_char', $key));					
			 	break;

			 case TYPE_INT:
			        $value = intval($result);
			 		if ($value < 1 || !is_int($value))
						eval(fetch_error_msg('not_integer', $key));					
			 	break;
				
			 case TYPE_TITLE:
					if (strlen($result) < 3)					
						eval(fetch_error_msg('short_field', $key));				 
			 	break;
				
			 case TYPE_BODY:
			 		$body = strip_html_tags($result);
					if (strlen($body) < 3)					
						eval(fetch_error_msg('short_field', $key));	
			 	break;
				
			 case TYPE_PASSWORD:
			 		if ($result == 'd41d8cd98f00b204e9800998ecf8427e' || empty($result))
						eval(fetch_error_msg('password_empty', $key));
					else if (strlen($result) < 6)
						eval(fetch_error_msg('password_short', $key));
					else if (isset($input['passwordconfirm_md5']) && $input['passwordconfirm_md5'] != $result && $key == 'password_md5')
						eval(fetch_error_msg('password_mismatch', $key));	
					else if (isset($_POST['passwordconfirm']) && $input['passwordconfirm'] != $result && $key == 'password')
						eval(fetch_error_msg('password_mismatch', $key));
			 	break;
		     ...
		     case TYPE_NAME:
					if (strlen($result) < 2)					
						eval(fetch_error_msg('short_field', $key));	
			 		elseif (!preg_match("#^[-0-9A-Za-z.' ]*$#", $result))
						eval(fetch_error_msg('invalid_char', $key));	
			 	break;
				
			case TYPE_EMAIL:
					if (!is_valid_email($result))
						eval(fetch_error_msg('email_invalid', $key)); 					
					elseif (isset($input['emailconfirm']) && $input['emailconfirm'] != $result)
						eval(fetch_error_msg('email_mismatch', $key));
					elseif (isset($exceptions['already_registered_email']) && already_registered_email($result))
						eval(fetch_error_msg('already_registered_email', $key));				
				break;
				
			case TYPE_CAPTCHA: 
				//	if (_CALLBACK_FUNCTION('fetch_captcha', CAPTCHA_PAGE))
					  { 
						include("includes/modules/captcha/securimage.php");
					    $img = new Securimage();
						if (!$img->check($_POST['captcha']))
							eval(fetch_error_msg('captcha', 'captcha'));				
					  }
				break;
		  }
	   }
	...
	* @return	array
	*/
	function clean_array_gpc($source, $variables, $exclude = array())
	{ 
		...
		if (is_array($build))
			$this->registry->current_vars = implode(',', $build);
	}
