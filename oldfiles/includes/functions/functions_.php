...
{
   global $languages;
   $result = query_array("select * from ".$table." where parent_id = ".$parent_id." and module !='block' order by sort");
...
function load_blocks($block = 'block')
{
  global $extra, $languageid;
  $blocks = query_array("select * from page_links where module = '".$block."'");
  for ($i=0; $i<count($blocks); $i++)
  	$extra[$blocks[$i]['name']] = $blocks[$i]['body_'.$languageid];
}
...
{
  global $Process; 
  $msg = defined('admin_module') ? $Process->admin_notify[$msg] : $Process->notify[$msg];
  $msg = parse_values($msg);
  return $msg;
}
...
function redirect($url=false, $para='')
{
   ...
   $url = $url == false && defined('REDIRECT') ? REDIRECT : $Process->http['index'].ADD_DIR.'/'.$admin.$url;
   exec_shut_down();
   if (!empty($para))
   	  $para = '/'.$para; 
   $url = empty($url) ? 'index' : $url.$para;
...
function update_website_stats()
{
  ...
  $Process->db->query_write("update ".TABLE_PREFIX."website_stats set ".$browser." = ".$browser." + 1, hits_total = hits_total + 1 where date = '".$date."'");
  if (mysql_affected_rows() == 0)
	  $Process->db->query_write("insert into ".TABLE_PREFIX."website_stats (date, ".$browser.", hits_total) values ('".$date."', 1, 1)");						   
}
...
function sendmail($toemail, $subject, $message, $use_template = true, $lang=false, $alternate_msg=false)
{
	...
	if ($Process->options['usemail'])
	{
		$mail =& new SMail($Process, $lang);
		if ($use_template)
			$message = $message;// fetch_email_template($message, $lang, $alternate_msg);
...
{
  global $Process;
  return "\$message = \"".parse_values($Process->email_msg[$msg]['message'])."\"; \$subject = \"".$Process->email_msg[$msg]['subject']."\" ;";	
}
...
{
    global $Process, $show, $extra, $phrase, $page, $cart, $languageid;

    require_once('includes/class/class.template.php');

    $Template = new smarty; 
	$Template->template_dir = $Process->config['Misc']['template'];
	$Template->assign('path', $Template->template_dir);
 	$Template->assign('path_img', $Template->template_dir.'/images');	
	$Template->assign('path_default_templates', $Template->template_dir);
...
//	$Template->assign('calendar', $Process->calendar->output_calendar(NULL, NULL, 'calendar', true));
	...
	$Template->assign('breadcrum', $page->breadcrum); 	
...
function fetch_template($filename, $lang=false)
{
	...
	return $Template->fetch($filename.'.tpl', null, null, false) ;	
}
...
{ 
	...
	if (defined('admin_module'))
	    ...
	    $use_theme ? $Template->display('theme.tpl') : $Template->display($filename.'.tpl');
	exit;
}
