...
function can_change_password($user)
{
  $found = query_first("select * from change_password where userid = ".intval($user['userid']));
  return empty($found);
}
...
function process_lostpassword()
{
  ...
  $Process->input->clean_array_gpc('p', array('email' => TYPE_STR));
  $email = $Process->GPC['email'];
  $Process->input->required(array('email' => TYPE_EMAIL));
  $Process->current_vars = '';
  if (found_email($email, $user))
	{ 
	   if (can_change_password($user))
		   email_lostpassword($user);
	   else
		   $Process->input->error_msg['email'] = fetch_error_msg('password_reset_already_requested');	   	   
	}
  else if (empty($Process->input->error_msg))
	    $Process->input->error_msg['email'] = fetch_error_msg('email_not_found');
}

function email_lostpassword($user, $action = 'lostpassword.php?success', $msg='reset_password')
{
  global $Process;
  $Process->current_vars = '';
  $password_reset_key = md5($user['email'].'-'.$user['userid']);
  $reset_password_expire_date = TIMENOW + $Process->config['change_password_timeout'];
  query("delete from change_password where userid = ".$user['userid']);
  save_input(
			array('table' 						=> 'change_password'),
			array('userid'						=> intval($user['userid']),
				  'password_reset_key'			=> $password_reset_key,
				  'password_reset_key_date'		=> $reset_password_expire_date), 'password_reset');
  $Process->GPC['userid'] = $user['userid'];
  $Process->GPC['website_reset_password_url'] = _ROOT_URL.'/admin/lostpassword.php?reset_key='.$password_reset_key.'&user='.intval($user['userid']);
  $Process->GPC['reset_password_expire_date'] = date('F d, Y - h A',$reset_password_expire_date);			
  sendmail_type($user['email'], $msg);
  redirect(_BASE_URL.$action);
}
...
function process_login()
{
  ...
  if (!verify_authentication($Process->GPC['login_email'], $Process->GPC['login_password'], $Process->GPC['login_md5password'], $Process->GPC['login_md5password_utf'], $Process->GPC['cookieuser'], true))
  ...
  else if ( (!defined('admin_module') || canviewcp($Process->GPC['login_email'])) && empty($Process->input->error_msg))
	{
		...
		if (defined('ADMINFORMS'))
			redirect('adminforms/forms.php');			
		else if (defined('admin_module'))
			redirect(_ROOT_URL);
...
function process_new_login($logintype, $cookieuser)
{
	...
	if ($Process->session->created == true AND $Process->session->vars['userid'] == 0)
		...
		$newsession = new Session($Process, '', $Process->userinfo['userid'], '', $Process->session->vars['styleid'], $Process->session->vars['language_id']);
